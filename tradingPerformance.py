...
class PerformanceEstimator:
    ...
    def computePnL(self):
        ...
        # Compute the PnL
        self.PnL = self.data.iloc[-1, self.data.columns.get_loc('Money')] - self.data.iloc[0, self.data.columns.get_loc('Money')]
        return self.PnL
    ...
    def computeProfitability(self):
        ...
        profit = 0
        loss = 0
        index = next((i for i in range(len(self.data.index)) if self.data.iloc[i, self.data.columns.get_loc('Action')] != 0), None)
        if index == None:
            self.profitability = 0
            self.averageProfitLossRatio = 0
            return self.profitability, self.averageProfitLossRatio
        money = self.data.iloc[index, self.data.columns.get_loc('Money')]

        # Monitor the success of each trade over the entire trading horizon
        for i in range(index+1, len(self.data.index)):
            if(self.data.iloc[i, self.data.columns.get_loc('Action')] != 0):
                delta = self.data.iloc[i, self.data.columns.get_loc('Money')] - money
                money = self.data.iloc[i, self.data.columns.get_loc('Money')]
                if(delta >= 0):
                    good += 1
                    profit += delta
                else:
        ...
        delta = self.data.iloc[-1, self.data.columns.get_loc('Money')] - money
        if(delta >= 0):
            good += 1
            profit += delta
        else:
